...
class AddressViewModel @Inject constructor(
    ...
    fun addAddress(address: com.example.gaseagua.data.Address){
        val validateInputs = validateInputs(address)

        if(validateInputs) {
            viewModelScope.launch { _addNewAddress.emit(Resource.Loading()) }
            firestore.collection("user").document(auth.uid!!)
                .collection("address").document().set(address).addOnSuccessListener {
            ...
            viewModelScope.launch { _error.emit("Preencha todos os campos") }
        }
    }
